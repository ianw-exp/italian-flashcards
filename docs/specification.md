# Italian Flashcards Web App — Software Specification

## Overview

A front-end-only flashcards web application to help users learn Italian vocabulary. Built using **Vite**, **React**, and **TypeScript**. The app uses **static data** to render flashcards grouped into categories. It supports study, quiz, and review modes, with performance tracked using in-browser state.
...
- Flashcards show a Italian word by default.
...
- Two types of quizzes:
  - **Multiple Choice**
    - One Italian word, 4 English options (1 correct + 3 distractors)
    - Distractors are statically defined per question
  - **Fill in the Blank**
    - Italian word shown, user types the English word
